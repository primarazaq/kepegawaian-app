...
<div x-data="{ disableCommPIC: false }">
    <button @click="disableCommPIC =!disableCommPIC" type="button" data-modal-toggle="popup-Del-CommentEMP"
        class="block py-2 w-full px-4 hover:bg-mainclr hover:bg-opacity-30 dark:hover:bg-gray-600 dark:hover:text-white">Hapus
        Respon</button>

    <div x-show="disableCommPIC"
        ...
        <div
            ...
            <div x-cloak @click="disableCommPIC = false" x-show="disableCommPIC"
                x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0"
                x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200 transform"
                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-40" aria-hidden="true"></div>

            <div x-cloak x-show="disableCommPIC" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <div class="text-center">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Komentarmu akan terhapus,
                        apakah anda yakin akan menghapus komentar anda??</h3>
                    <div class="flex justify-center">
                        <form action="/pic/home/dashboard/{{ $data->id }}" method="post">
                            @method('delete')
                            @csrf
                            <input type="text" name="response" value="1" hidden>
                            <button type="submit"
                        ...
                        <button @click="disableCommPIC =!disableCommPIC" type="button"
