...
<div x-data="{ notif: false }">
        ...
        @if (auth()->user()->level=="pic")
            <button @click="notif =!notif" type="button" class="absolute right-4 top-3 p-2 ml-3 text-sm" aria-controls="navbar-default" aria-expanded="false">
                {{-- icon ketika muncul notifikasi --}} 
                @if (count($notifPIC) > 0 )
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" class="animate-pulse absolute w-4" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
    ...
    <div x-show="notif" class="fixed overflow-y-auto overflow-x-hidden -my-8 top-0 right-0 z-50 h-modal justify-end items-end" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="relative p-4 h-full md:h-auto">
            ...
            <div class="flex px-2 py-2 rounded-t dark:border-gray-600">
                ...
                <button @click="notif = false" type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-full border-2 border-black text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="modalNotification">
