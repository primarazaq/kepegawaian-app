...
@section('content')
    ...
    <div class="flex pb-3 justify-between pl-14 pr-32">
        <div class="bg-mainclr w-40 p-1 ml-16 rounded-full text-white flex">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 fill-white ml-3 mr-7" viewBox="0 0 512 512">
    ...
    <div class="flex mx-auto max-w-fit h-fit border-4 rounded-lg border-mainclr">
        ...
        <div class="basis-1/2 bg-gray-100 rounded-l-lg p-5">
            ...
            <form method="post" action="/pic/home/dashboard/{{ $data->task_id }}" enctype="multipart/form-data">
                ...
                @csrf
                <div>
                    <label for="t_title" class="block ml-4 text-lg font-semibold text-gray-800 dark:text-gray-300">Title
                        Task</label>
                    <input type="text" id="t_title" name="t_title" placeholder="Tuliskan Judul tugas disini..."
                        class="block w-full text-3xl text-gray-900 bg-gray-100 border-gray-100 :focus-visible" required
                        value="{{ old('t_title', $data->t_title) }}">
                </div>
                <div class="mb-2">
                    ...
                    <div class="flex">
                        ...
                        @if ($data->t_file)
                            <div id="ubahfile" class="flex">
                                <a href="{{ asset('storage/task-file/' . $data->t_file) }}"
                                    class="text-mainclr hover:text-teal-600 underline ml-5 -mt-1" target="_blank">lihat file
                                    sebelumnya</a>
                                ...
                                <button id="ubah" type="button" class="ml-5 -mt-3 flex items-center">
                                    <svg version="1.1" id="Capa_1" x="0px" y="0px" width="15px"
                                        height="15px" viewBox="0 0 494.936 494.936"
                ...
                <input
                    ...
                    aria-describedby="t_file_help" id="t_file" name="t_file" type="file"
                    value="{{ old('t_file', $data->t_file) }}">
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="t_file_help">*jpeg , jpg, png, docx, doc, pptx,
                    ppt, xlsx, xls, pdf, zip, rar (MAX FILE 10MB).</p>
                @endif


        </div>
        <div class="mb-4">
            <label for="t_body" class="ml-4 text-lg font-semibold text-gray-900 dark:text-gray-300">Description
                Task</label>
    ...
    <div class=" bg-white p-5 rounded-r-lg">
        ...
        <div class="flex mb-4">
            ...
            <div class="font-extrabold text-lg ml-5">
                <input type="text" value="{{ $data->t_status }}" name="t_status" class="block bg-white border-white"
                    readonly>
            </div>
        </div>
        <div class="flex mb-4">
            ...
            <div class="t_priority-dropdown">
                ...
                <select id="mydropdownlist" name="t_priority" required>
                    ...
                    @foreach ($priority as $item)
                        <option value="{{ $item }}" @if ($data->t_priority == $item) selected ="selected" @endif>
                            {{ $item }}</option>
                    @endforeach
        ...
        <div class="flex mb-4">
            ...
            <div class="user_receiver_id-dropdown">
                <select data-placeholder="Select People" name="user_receiver_id[]" multiple
                    class="chosen-select form-control" style="width: 156px" multiple>
                    <option></option>
                    @if (count($arrID) > 1)
                        @foreach ($arrID as $id)
                        ...
                        @foreach ($arrID as $id)
                            @foreach ($pegawai as $item)
                                ...
                                <option value="{{ $item->id }}"
                                    @if ($id == $item->id) selected ="selected" @endif> {{ $item->name }}
                                </option>
                            @endforeach
                        @endforeach
                    @endif
        ...
        <div class="flex mb-4">
            ...
            <div date-rangepicker class="items-center ml-2">
                <div class="relative">
                    <input name="t_due_date" type="dateTime-local"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Select date end" required value="{{ old('t_due_date', $data->t_due_date) }}">
                </div>
            </div>
        </div>
        <div class="flex mb-4">
        ...
        @if (session()->has('success'))
            <div id="alert-3" class="flex justify-between p-4 mb-4 bg-green-100 rounded-lg dark:bg-green-200"
                role="alert">
        ...
        <div class="flex items-center justify-center mt-28">
            <a href="/pic/home/dashboard">
                <button type="button"
                    class="text-gray-900 bg-white border border-gray-300 hover:bg-gray-100 font-medium rounded-lg text-sm px-5 py-2 mr-4 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600">Cancel</button>
            </a>
            <button type="submit"
